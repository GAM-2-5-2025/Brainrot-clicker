...
button_rect3 = brain.get_rect(topleft=(1350, 550))
button_hover_rect3 = brain_hover.get_rect(topleft=(1350, 550))
...
def main():
    ...
    while run:
        ...
        ltf = myFont2.render(f"Low Taper Fade: {buy_3}", 1, white)
        ltf_price = myFont2.render(f"Price: {value_3}", 1, white)
...
#low taper fade button
        ...
        if br < value_4:
            cursor_no_money = myFont2.render(f"Low Taper Fade: {buy_4}", 1, black)
            cursor_price_no_money = myFont2.render(f"Price: {value_4}", 1, black)
